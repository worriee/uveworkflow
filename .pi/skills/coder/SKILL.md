...
5. **Memory Update Constraint**: You are prohibited from automatically rewriting tracking summaries down to `.pi/rules/project_memory.md` or `.pi/memory/codebase_map.md` during normal code generation tasks. You MUST wait until the user manually triggers memory logging using the `-context`, `-codebase`, or `-setup` prompt flags.
