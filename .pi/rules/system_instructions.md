...
- **Absolute Rule Folder Isolation**: You are strictly forbidden from creating any new folders or files inside the `.pi/rules/` directory. You must only utilize, read, and write to the existing memory files (`project_memory.md`) in this directory and the memory files inside `.pi/memory/` and `.pi/archives/` directory.
...
- **Prohibited Background Mutating**: Do not alter, overwrite, or touch `.pi/rules/project_memory.md` or any files inside `.pi/memory/` during feature development or bug patches unless the user explicitly included the specific trigger flags (`-context`, `-error`, `-codebase`, `-setup`, `-archive`, `-obsidian`).
- **Command Flag Processing Execution**:
  - If `-init` is prompted: Read `.pi/workspace.json`. If `workspace_id` equals `"uninitialized"`, prompt the user: `"Enter project name for workspace initialization:"`. Generate `workspace_id` as a lowercase-hyphenated slug of the project name. Set `project_name` to the user-provided name. Set `initialized_at` to the current PST timestamp via `Get-Date -Format "MMMM dd, yyyy, hh:mm tt PST"`. Set `initialized_by` to the exact AI model ID of this session (extract from system prompt). Write the updated values back to `.pi/workspace.json`. If `workspace_id` is already set to a non-`"uninitialized"` value, HALT initialization and log `WORKSPACE ALREADY INITIALIZED: {project_name}`. Do NOT modify `workspace.json` under any other circumstance.
  - If `-context` is prompted: Run structural inspection and update `.pi/rules/project_memory.md` together with its timestamp.
  - If `-error` is prompted: Analyze debugging traces and update `.pi/memory/error_memory.md` together with its timestamp.
  - If `-codebase` is prompted: Log directory maps and file utilities inside `.pi/memory/codebase_map.md` together with its timestamp. **Strict Exclusion Constraint**: You are strictly forbidden from documenting, explaining, or listing any files or folders ignored by the project's `.gitignore` file within the codebase map tracker.
  ...
  - If `-obsidian` is prompted: Execute the Mandatory Obsidian Vault Synchronization Protocol. Read `.pi/workspace.json`; if `workspace_id` is `"uninitialized"`, HALT and demand `-init` first. Resolve the vault path from `.pi/rules/project_memory.md`, or ASK the user and persist the confirmed path there (LIFO). Create `<vault>/<project_name>/` (exact `project_name` from workspace.json), mirror the active template root 1:1 into it, EXCLUDING `node_modules/` and `.git/`. Overwrite existing files unconditionally. Report the copied file list to the user. Any path fabrication, silent skip, or partial copy is a CRITICAL VIOLATION.
- **Ultimate Historical Error Preservation & Retention Formatting**: When processing the `-error` or `-setup` flags to update `.pi/memory/error_memory.md`, you are under strict professional command to NEVER alter, wipe, truncate, or delete any data located inside 'Section 2: Historical & Resolved Errors'. Past resolved bugs are critical progress milestones and immutable project history. You must only append new resolutions to that section.
...
- **Recovery Priority**: When a context window reset happens, prioritize executing tool read commands on the `.pi/rules/.clinerules` configuration layout files immediately.
...
- **High-Detail Beginner Clarity**: Every update pushed to `.pi/rules/project_memory.md` and `.pi/memory/codebase_map.md` must thoroughly detail the specific role of the components and etc, explaining who depends on them in a highly readable, simplified format.
...
- **Vulnerability Logging Protocol**: When running under `-s`, you must safely open `.pi/memory/security_memory.md` and log attack patterns strictly within `## 1. Active & Unresolved Vulnerabilities` using a clear LIFO ordering format.
...
- **Workspace Initialization Mandate**: When processing any command flag, first check `.pi/workspace.json`. If `workspace_id` equals `"uninitialized"`, prioritize initialization before executing the requested task. Log the initialization event in the command output.
- **Archival Execution Protocol**: When the user explicitly prompts the `-archive` flag, scan all eligible memory files and archive oldest entries when sections exceed 10 entries. Eligible files are: `.pi/memory/error_memory.md`, `.pi/memory/implementation_memory.md`, `.pi/memory/security_memory.md`, `.pi/memory/review_memory.md`, and `.pi/memory/test_memory.md`. **Excluded from archival**: `.pi/memory/codebase_map.md` and `.pi/rules/project_memory.md`. If threshold is exceeded:
...
- **Zero-Tolerance Rule Tampering**: You are strictly prohibited from mutating, editing, adding, or deleting any instruction, persona, or framework layout file inside `.pi/rules/` or `.pi/skills/` (`system_instructions.md`, `.clinerules`, `.pi/skills/orchestrator/SKILL.md`, `.pi/skills/planner/SKILL.md`, `.pi/skills/coder/SKILL.md`, `.pi/skills/debugger/SKILL.md`, `.pi/skills/ask/SKILL.md`, `.pi/skills/secure/SKILL.md`, `.pi/skills/reviewer/SKILL.md`, `.pi/skills/tester/SKILL.md`).
- Permitted Writes: Your modification authority is strictly limited to updating dynamic state indicators within `.pi/rules/project_memory.md`, `.pi/memory/error_memory.md`, `.pi/memory/codebase_map.md`, `.pi/memory/implementation_memory.md`, `.pi/memory/security_memory.md`, `.pi/memory/review_memory.md`, and `.pi/memory/test_memory.md`.
