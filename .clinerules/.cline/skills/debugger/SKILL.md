...
2. **On-Demand Error Logging**: Do not write structural analytics into `.clinerules/error_memory.md` automatically during testing routines. You must only interact with, parse, or write into the error memory module when the user explicitly requests it via the `-error` or `-setup` flags.
