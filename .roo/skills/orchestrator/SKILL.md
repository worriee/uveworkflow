...
2. **Delegation**: Explicitly guide the flow of execution based on single-letter flags or workflow state:
   - Direct to `.roo/skills/planner/SKILL.md` (flag: `-p`) for architectural mapping and specification gathering.
   - Direct to `.roo/skills/coder/SKILL.md` (flag: `-c`) for clean feature implementation.
   - Direct to `.roo/skills/debugger/SKILL.md` (flag: `-d`) for addressing unit test failures or integration bugs.
   - Direct to `.roo/skills/ask/SKILL.md` (flag: `-a`) for querying complex documentation or existing patterns.
   - Direct to `.roo/skills/secure/SKILL.md` (flag: `-s`) for running exploit modeling, computing security ratings, and identifying architectural vulnerabilities.
3. **State Integration**: Monitor task progress by cross-referencing completed sub-tasks against the project's state memory (`.roo/rules/project_memory.md`) and ongoing bugs tracked in (`.roo/memory/error_memory.md`).
