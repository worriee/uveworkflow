...
- **Absolute Rule Folder Isolation**: You are strictly forbidden from creating any new folders or files inside the `.roo/rules/` directory. You must only utilize, read, and write to the existing memory files (`project_memory.md`) in this directory and the memory files inside `.roo/memory/` and `.roo/archives/` directory.
...
- **Prohibited Background Mutating**: Do not alter, overwrite, or touch `.roo/rules/project_memory.md` or any files inside `.roo/memory/` during feature development or bug patches unless the user explicitly included the specific trigger flags (`-context`, `-error`, `-codebase`, `-setup`, `-archive`, `-obsidian`).
- **Command Flag Processing Execution**:
  - If `-init` is prompted: Read `.roo/workspace.json`. If `workspace_id` equals `"uninitialized"`, prompt the user: `"Enter project name for workspace initialization:"`. Generate `workspace_id` as a lowercase-hyphenated slug of the project name. Set `project_name` to the user-provided name. Set `initialized_at` to the current PST timestamp via `Get-Date -Format "MMMM dd, yyyy, hh:mm tt PST"`. Set `initialized_by` to the exact AI model ID of this session (extract from system prompt). Write the updated values back to `.roo/workspace.json`. If `workspace_id` is already set to a non-`"uninitialized"` value, HALT initialization and log `WORKSPACE ALREADY INITIALIZED: {project_name}`. Do NOT modify `workspace.json` under any other circumstance.
  - If `-context` is prompted: Run structural inspection and update `.roo/rules/project_memory.md` together with its timestamp.
  - If `-error` is prompted: Analyze debugging traces and update `.roo/memory/error_memory.md` together with its timestamp.
  - If `-codebase` is prompted: Log directory maps and file utilities inside `.roo/memory/codebase_map.md` together with its timestamp. **Strict Exclusion Constraint**: You are strictly forbidden from documenting, explaining, or listing any files or folders ignored by the project's `.gitignore` file within the codebase map tracker.
...
- **High-Detail Beginner Clarity**: Every update pushed to `.roo/rules/project_memory.md` and `.roo/memory/codebase_map.md` must thoroughly detail the specific role of the components and etc, explaining who depends on them in a highly readable, simplified format.
...
- **Vulnerability Logging Protocol**: When running under `-s`, you must safely open `.roo/memory/security_memory.md` and log attack patterns strictly within `## 1. Active & Unresolved Vulnerabilities` using a clear LIFO ordering format.
...
- **Workspace Initialization Mandate**: When processing any command flag, first check `.roo/workspace.json`. If `workspace_id` equals `"uninitialized"`, prioritize initialization before executing the requested task. Log the initialization event in the command output.
- **Archival Execution Protocol**: When the user explicitly prompts the `-archive` flag, scan all eligible memory files and archive oldest entries when sections exceed 10 entries. Eligible files are: `.roo/memory/error_memory.md`, `.roo/memory/implementation_memory.md`, `.roo/memory/security_memory.md`, `.roo/memory/review_memory.md`, and `.roo/memory/test_memory.md`. **Excluded from archival**: `.roo/memory/codebase_map.md` and `.roo/rules/project_memory.md`. If threshold is exceeded:
...
- **Zero-Tolerance Rule Tampering**: You are strictly prohibited from mutating, editing, adding, or deleting any instruction, persona, or framework layout file inside `.roo/rules/` or `.roo/skills/` (`system_instructions.md`, `.clinerules`, `.roo/skills/orchestrator/SKILL.md`, `.roo/skills/planner/SKILL.md`, `.roo/skills/coder/SKILL.md`, `.roo/skills/debugger/SKILL.md`, `.roo/skills/ask/SKILL.md`, `.roo/skills/secure/SKILL.md`, `.roo/skills/reviewer/SKILL.md`, `.roo/skills/tester/SKILL.md`).
- Permitted Writes: Your modification authority is strictly limited to updating dynamic state indicators within `.roo/rules/project_memory.md`, `.roo/memory/error_memory.md`, `.roo/memory/codebase_map.md`, `.roo/memory/implementation_memory.md`, `.roo/memory/security_memory.md`, `.roo/memory/review_memory.md`, and `.roo/memory/test_memory.md`.
